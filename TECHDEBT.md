...
- **Стало (27.06):** vitest-харнессы с ОБЕИХ сторон + **56 юнит-тестов** на чистое ядро:
  - server `extraction.test.ts` — 40 (дедуп/нормализация/мета/жанр/настроение/заголовки), без БД.
  - client `editorUtils.test.tsx` — 16 (многословный матчинг, разбор заголовков). vitest+jsdom подняты.
