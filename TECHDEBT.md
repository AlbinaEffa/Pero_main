...
- **Стало (27.06):** vitest-харнессы с ОБЕИХ сторон + **76 юнит-тестов** на чистое ядро:
  - server `extraction.test.ts` (40) + `html.test.ts` (11: конвертация экспорта txt/md, счёт слов) — без БД.
  - client `editorUtils.test.tsx` (16) + `storyBibleDedup.test.ts` (9: дедуп-детектор, защита от ложных категорий). vitest+jsdom подняты.
- **Осталось:** расширить покрытие (planLimits/quota — нужен DB-мок или интеграция; компоненты-хуки;
  paragraph-diff recheck); **Playwright e2e** на петлю «импорт→чтение→нестыковка». **Effort:** M→L. **Priority:** P1 к бете.
