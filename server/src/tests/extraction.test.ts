...
describe('isMetaEntity (фильтр местоимений и эхо-полей)', () => {
  ...
  it('родовые слова-роли (Героиня/Протагонист/Рассказчик) — мета, а не имя', () => {
    for (const r of ['Героиня', 'герой', 'Главная героиня', 'Главный герой', 'Протагонист', 'Рассказчик', 'персонаж']) {
      expect(isMetaEntity({ name: r }), r).toBe(true);
    }
    // плейсхолдер-описание «персонаж с именем, упомянутым в тексте» — тоже мета
    expect(isMetaEntity({ name: 'Кто-то', description: 'Персонаж с именем, упомянутым в тексте.' })).toBe(true);
  });
